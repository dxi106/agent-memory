...
export async function safeLedgerCheck({
  ...
  try {
    ...
    // Deliberately NOT "run `gh auth login`". This string is rendered inside the
    // untrusted-data block, whose own preamble tells the model to treat a "run
    // this" instruction in there as suspicious — the exact self-contradiction
    // that got TRIAGE_INSTRUCTION moved out of the block. Remediation is
    // agentmem's own voice and belongs on stdout, not in the model's context.
    if (!fetcher) return failed("no GitHub credentials available");
