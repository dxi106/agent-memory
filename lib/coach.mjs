...
export function buildCoachingPrompt({ signals, lessons, knowledge, usageStats, model = DEFAULT_MODEL, config = {} }) {
  ...
  return {
    model,
    // SOU-40: this was a hardcoded 4096, and the 2026-08-07 run hit it exactly
    // — seven complete recommendations cut mid-string and thrown away. The
    // check that catches a future truncation reads `req.max_tokens` straight
    // back off this object, so the budget and the guard cannot drift.
    max_tokens: config.max_output_tokens ?? DEFAULT_MAX_OUTPUT_TOKENS,
    system,
 ...
 *
 * THROWS `ModelOutputError` when the output cannot be read at all (SOU-40).
 * It used to return an empty list instead, which reported a broken run as a
 * quiet one. Being best-effort about *content* is right; being best-effort
 * about *legibility* is how 84k tokens of input became "wrote 0
 * recommendation(s)" and exit 0.
 *
 * @returns {{recommendations: object[], proposed: number, rejected: string[]}}
 *   `proposed` counts what the model actually offered and `rejected` names
 *   what validation turned away — together they make every proposal
 *   accounted for.
 */
export function parseCoachResponse(raw, { minEvidence = DEFAULT_MIN_EVIDENCE } = {}) {
  ...
  const parsed = parseModelJson(raw, "coaching");
  ...
  for (const r of list) {
    ...
    if (ok) out.push(ok);
    else rejected.push(typeof r?.id === "string" ? r.id : "(no id)");
  }
  return { recommendations: out, proposed: list.length, rejected };
}
...
async function writeCoachingLog(home, payload) {
  ...
  const lines = [
    ...
    `- recs_rejected: ${payload.rejected?.length ?? 0}`,
    ...(payload.rejected?.length ? [`- rejected_ids: ${describeRejected(payload.rejected)}`] : []),
    `- dry_run: ${payload.dryRun}`,
    // flattenField again at the boundary, not only at the source: this is a
    // single-line list item in a markdown document, and a newline anywhere in
    // the message silently restructures the file.
...
export async function runCoachingPass({ home, client, sinceDays, dryRun = false, lessons, runNotifier } = {}) {
  ...
  // Whatever goes wrong below, the raw output still reaches reflections/ —
  // that log is the only forensic record of what a paid call produced, and
  // losing it is how SOU-40 stayed invisible for ten weeks.
  const failRun = async (err, counts = {}) => {
    if (!dryRun) {
      try {
        await writeCoachingLog(home, {
          model,
        ...
        // Report it, but never let it mask the failure we are actually raising.
        console.error(`coach: could not write the failure log: ${logErr.message}`);
      }
    }
    throw err;
  };
  ...
  let parsed;
  try {
    // Order matters: a truncated response is often *also* unparseable, and
    // "we ran out of output budget" is the more actionable of the two.
    assertNotTruncated(response, req.max_tokens, "coaching");
    parsed = parseCoachResponse(raw, { minEvidence });
  } catch (e) {
    await failRun(e);
  }
  ...
  // Every proposal must land in exactly one bucket: written, or skipped for a
  // reason we recorded. A run that consumed input and produced neither threw
  // the model's work away — the same user-visible outcome as SOU-40, reached
  // by a different route (validation rejecting all of it rather than a parse
  // failure). Fail closed and say which ids died.
  const accountedFor = written.length + skippedDismissed + skippedSnoozed + skippedExisting;
  if (parsed.proposed > 0 && accountedFor === 0) {
    await failRun(
      new ModelOutputError(
        `coaching: the model proposed ${parsed.proposed} recommendation(s) and none were written ` +
        `or skipped for a recorded reason — all were rejected by validation ` +
        `(${describeRejected(parsed.rejected)}). Nothing was written. ` +
        `Check min_evidence_per_recommendation and the raw output in the run log.`,
        { kind: "unaccounted" },
      ),
      { proposedCount: parsed.proposed, rejected: parsed.rejected },
    );
  }
  ...
  if (!dryRun) {
    logPath = await writeCoachingLog(home, {
      model,
      signalsCount: signals.length,
      proposedCount: parsed.proposed,
      writtenCount: written.length,
      skippedDismissed,
      skippedSnoozed,
      skippedExisting,
      // A partial loss — 7 proposed, 1 written, 6 turned away — does not trip
      // the guard above, because one proposal WAS accounted for. Without this
      // line it would leave no record either, which is the same silent-loss
      // class this ticket exists to close, one case narrower.
