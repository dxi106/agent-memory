 ...
 * protect it.
 */
// Per the Messages API's own classification. `pause_turn` is deliberately NOT
// here: it means the server-tool loop hit its iteration limit, not that the
// model finished — Anthropic classifies it as incomplete. Neither pass sends
// `tools` today, so the tool-related values are dormant; that is exactly why
// they are pinned by tests rather than left to be re-derived later.
const COMPLETE_STOP_REASONS = new Set(["end_turn", "stop_sequence", "tool_use"]);
const TRUNCATING_STOP_REASONS = new Set(["max_tokens", "model_context_window_exceeded"]);

export function isTruncated(response, maxTokens) {
  if (!response || typeof response !== "object") return false;
  if (TRUNCATING_STOP_REASONS.has(response.stop_reason)) return true;
...
export function parseModelJson(raw, label = "model") {
  ...
  // Anything that is not a plain object is unreadable, and every one of these
  // shapes degrades to the SAME silent "zero items" if it gets through:
  //   - a bare scalar (`42`, `"str"`, `null`) has no fields to read;
  //   - an ARRAY is the dangerous one, because `typeof [] === "object"` sails
  //     past a naive check and then `parsed.recommendations` is undefined. A
  //     model that returns the bare list instead of the `{"recommendations":
  //     [...]}` wrapper is ordinary behaviour, and the accounted-for guard
  //     cannot catch it: `proposed` is 0, so the guard's precondition is false.
  //     Measured: a two-recommendation array produced {recs: 0, proposed: 0}.
  if (parsed === undefined || parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new ModelOutputError(
      `${label}: could not read the model's output as a JSON object` +
      // The position when V8 gives one, the size always. V8 omits the position
      // for a failure at character 0 — the common "the model wrote prose" case —
      // and its message quotes the input verbatim, so it can never be echoed.
      (failedAt
        ? ` (invalid JSON at character ${failedAt} of ${stripped.length})`
        : ` (${stripped.length} characters, unreadable as JSON)`) +
