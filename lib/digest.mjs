...
export function selectDigestItems(candidates, cap = DEFAULT_DIGEST_CAP) {
  ...
  const usable = candidates.filter((c) => c?.meta?.id);
  return orderCandidates(usable).slice(0, Math.max(0, cap));
}
...
function render({ today, items, warning, pendingTotal }) {
  ...
  if (items.length > 0) {
    ...
    items.forEach((c, i) => {
      const id = flattenField(c.meta.id, 120).replace(/[^A-Za-z0-9_-]/g, "");
