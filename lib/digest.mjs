...
export async function runDigest(
  ...
  // An error is news about the TOOL, not about the ledger, and it recurs every
  // single run on a machine that simply has no credentials. Left in the
  // `hasLedgerNews` test it manufactured a digest on days with nothing to say,
  // so every session received a block whose entire content was a tooling
  // complaint. The operator still learns about it — on stdout. When a digest is
  // being written anyway, the note IS included, so the section is never
  // silently partial.
  const hasLedgerNews =
    Boolean(ledger) && (ledger.missing.length > 0 || (!ledger.complete && !ledger.error));
