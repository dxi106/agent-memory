import { readFile, readdir, writeFile, mkdir } from "node:fs/promises";
import { join } from "node:path";
import {
  paths,
  loadConfig,
  listLessons,
  listCandidates,
  writeCandidate,
} from "./storage.mjs";
import { readSignalsSince } from "./signals.mjs";
import { serializeLesson, flattenField } from "./lesson.mjs";
import {
  REFLECTION_PROMPT_VERSION,
  reflectionSystemPreamble,
  REFLECTION_USER_PROMPT,
} from "./reflection-prompt.mjs";
import { syncToObsidian } from "./obsidian.mjs";
import {
  DEFAULT_MAX_OUTPUT_TOKENS,
  ModelOutputError,
  assertNotTruncated,
  describeRejected,
  parseModelJson,
} from "./model-output.mjs";

const SAFE_ID = /^[A-Za-z0-9_-]+$/;
const VALID_CATEGORIES = new Set(["behavioral", "code", "workflow"]);
const DEFAULT_MODEL = "claude-sonnet-4-6";
const DEFAULT_LOOKBACK = 7;
const DEFAULT_MIN_SIGNALS = 3;
const CANDIDATE_CONFIDENCE = 0.35;
// Measured 2026-08-05: the pass was proposing ~9.7 candidates a night (max 18),
// which is what produced the 4-week untriaged backlog. See SOU-31.
const DEFAULT_MAX_CANDIDATES = 3;

function timestampStamp() {
  const d = new Date();
  const pad = (n) => String(n).padStart(2, "0");
  return `${d.getUTCFullYear()}-${pad(d.getUTCMonth() + 1)}-${pad(d.getUTCDate())}-${pad(d.getUTCHours())}-${pad(d.getUTCMinutes())}-${pad(d.getUTCSeconds())}`;
}

function clamp(n, lo = 0, hi = 1) {
  return Math.max(lo, Math.min(hi, n));
}

async function loadKnowledgeBase(home) {
  const kdir = paths(home).knowledge;
  let files;
  try {
    files = await readdir(kdir);
  } catch {
    return "";
  }
  const out = [];
  for (const f of files.sort()) {
    if (!f.endsWith(".md")) continue;
    const text = await readFile(join(kdir, f), "utf8");
    out.push(`## ${f}\n\n${text}`);
  }
  return out.join("\n\n");
}

function formatLessonsForPrompt(lessons) {
  if (lessons.length === 0) return "_(none)_";
  return lessons
    .map((l) => {
      const m = l.meta || {};
      const conf = (m.confidence ?? 0).toFixed(2);
      return `### ${m.id} — ${m.title} (${m.category}, ${conf})\n${(l.body || "").trim()}`;
    })
    .join("\n\n");
}

function formatSignalsForPrompt(signals) {
  if (signals.length === 0) return "_(none)_";
  // Keep it terse — a one-line summary per signal is enough for the model
  // to spot patterns without blowing up the input.
  return signals
    .map((s) => {
      const cwd = s.cwd ? ` cwd=${s.cwd}` : "";
      return `- [${s.ts}] ${s.type}${cwd}: ${(s.summary || "").trim()}`;
    })
    .join("\n");
}

/**
 * Assemble a Messages API request shaped for prompt caching:
 * - tools: none
 * - system: [preamble, existing lessons, knowledge base]  (stable; cached)
 * - messages: [user with recent signals]                  (volatile; per-run)
 *
 * The last system block carries an ephemeral cache_control marker so the
 * stable prefix (preamble + lessons + KB) gets cached across nightly runs.
 */
export async function buildReflectionRequest({
  home,
  signals,
  model = DEFAULT_MODEL,
  lessons,
  maxCandidates = DEFAULT_MAX_CANDIDATES,
  maxOutputTokens = DEFAULT_MAX_OUTPUT_TOKENS,
}) {
  const activeLessons = lessons || (await listLessons(home));
  const kb = await loadKnowledgeBase(home);

  const system = [
    { type: "text", text: reflectionSystemPreamble(maxCandidates) },
    {
      type: "text",
      text: `# EXISTING LESSONS\n\n${formatLessonsForPrompt(activeLessons)}`,
    },
    {
      type: "text",
      text: `# KNOWLEDGE BASE\n\n${kb || "_(none)_"}`,
      // ↓ Cache breakpoint sits on the LAST stable block, so the preamble +
      // lessons + KB are all cached together. Signals (volatile) go in messages.
      cache_control: { type: "ephemeral" },
    },
  ];

  const user = `${REFLECTION_USER_PROMPT}\n\n# RECENT SIGNALS\n\n${formatSignalsForPrompt(signals)}`;

  return {
    model,
    // SOU-40: this was a hardcoded 4096, the same cap that truncated coach on
    // 2026-08-07. Reflect had stayed under it only because SOU-31 capped
    // candidates at 3 per run — protected by accident, not by design.
    max_tokens: maxOutputTokens,
    system,
    messages: [{ role: "user", content: user }],
  };
}

function extractText(response) {
  const blocks = response?.content || [];
  return blocks
    .filter((b) => b && b.type === "text" && typeof b.text === "string")
    .map((b) => b.text)
    .join("\n")
    .trim();
}

function sanitizeCandidate(c) {
  if (!c || typeof c !== "object") return null;
  if (typeof c.id !== "string" || !SAFE_ID.test(c.id)) return null;
  if (!VALID_CATEGORIES.has(c.category)) return null;
  if (typeof c.title !== "string") return null;
  // The one model-controlled field with no structural guard. Flatten and bound
  // it here, at the write chokepoint, the same way id and category are gated.
  // One emptiness check, after flattening — flattenField strips exactly the
  // character set trim() does, so a separate pre-check could never fire.
  const title = flattenField(c.title);
  if (!title) return null;

  const scope = Array.isArray(c.scope) && c.scope.length > 0
    ? c.scope.filter((s) => typeof s === "string" && s.trim())
    : ["*"];

  const ruleLine = typeof c.rule === "string" && c.rule.trim() ? c.rule.trim() : "";
  const whyLine = typeof c.why === "string" && c.why.trim() ? c.why.trim() : "";
  const body = [ruleLine, whyLine].filter(Boolean).join("\n\n") || "**Rule:** _(empty — model proposed without body)_";

  // `created` stays date-only for backwards compatibility — every existing
  // candidate has it and the digest still reads it as a fallback. `created_at`
  // is additive and forward-only (SOU-30): it gives new records true arrival
  // order, which a date cannot, since one reflect pass writes several
  // candidates on the same day.
  const now = new Date().toISOString();

  return {
    meta: {
      id: c.id,
      title,
      category: c.category,
      confidence: CANDIDATE_CONFIDENCE,
      created: now.slice(0, 10),
      created_at: now,
      source: "reflection",
      scope: { repos: scope },
    },
    body,
  };
}

/**
 * Apply a rescore delta to an existing lesson file, in place.
 * confirm = +evidence_for_step (config), contradict = -evidence_against_step.
 */
async function rescoreLesson(home, id, delta, config) {
  const lessons = await listLessons(home);
  const lesson = lessons.find((l) => l.meta.id === id);
  if (!lesson) return false;

  const c = config.confidence || {};
  const step = delta === "confirm"
    ? c.evidence_for_step ?? 0.15
    : -(c.evidence_against_step ?? 0.25);
  const next = clamp((lesson.meta.confidence ?? 0) + step);

  lesson.meta.confidence = Number(next.toFixed(2));
  lesson.meta.last_evidence = new Date().toISOString().slice(0, 10);

  await writeFile(lesson.path, serializeLesson(lesson));
  return true;
}

async function writeReflectionLog(home, payload) {
  const stamp = timestampStamp();
  const dir = paths(home).reflections;
  await mkdir(dir, { recursive: true });
  const file = join(dir, `${stamp}.md`);
  const lines = [
    `# Reflection ${stamp}`,
    "",
    `- prompt_version: ${REFLECTION_PROMPT_VERSION}`,
    `- model: ${payload.model}`,
    `- signals_considered: ${payload.signalsCount}`,
    `- candidates_proposed: ${payload.proposedCount}`,
    `- candidates_written: ${payload.writtenCount}`,
    `- rescored: ${payload.rescoredCount}`,
    `- candidates_rejected: ${payload.rejected?.length ?? 0}`,
    ...(payload.rejected?.length ? [`- rejected_ids: ${describeRejected(payload.rejected)}`] : []),
    `- dry_run: ${payload.dryRun}`,
    // flattenField at the boundary as well as at the source: this is a
    // single-line markdown list item, and a newline anywhere in the message
    // silently restructures the file.
    ...(payload.failure ? [`- failure: ${flattenField(payload.failure, 600)}`] : []),
    "",
    "## Raw model output",
    "",
    "```",
    payload.rawOutput || "(empty)",
    "```",
  ];
  await writeFile(file, lines.join("\n") + "\n");
  return file;
}

/**
 * Run a reflection pass.
 *
 * @param {object} opts
 * @param {string} opts.home  - agentmem home
 * @param {object} opts.client - Anthropic SDK client (or a stub with .messages.create)
 * @param {boolean} [opts.dryRun]
 *
 * Returns { skipped, reason, candidates, rescored, logPath }.
 */
export async function runReflection({ home, client, dryRun = false }) {
  const config = await loadConfig(home).catch(() => ({}));
  const rconfig = config.reflection || {};
  const lookback = rconfig.lookback_days ?? DEFAULT_LOOKBACK;
  const minSignals = rconfig.min_signals_to_reflect ?? DEFAULT_MIN_SIGNALS;
  const model = rconfig.model || DEFAULT_MODEL;
  const maxCandidates = rconfig.max_candidates_per_run ?? DEFAULT_MAX_CANDIDATES;
  const maxOutputTokens = rconfig.max_output_tokens ?? DEFAULT_MAX_OUTPUT_TOKENS;

  const signals = await readSignalsSince(paths(home).signals, lookback);

  if (signals.length < minSignals) {
    return {
      skipped: true,
      reason: `min_signals_to_reflect=${minSignals}, got ${signals.length}`,
      candidates: [],
      rescored: [],
    };
  }

  const req = await buildReflectionRequest({ home, signals, model, maxCandidates, maxOutputTokens });

  const response = await client.messages.create(req);
  const raw = extractText(response);

  // SOU-40: a truncated or unreadable response is a FAILED run, not an empty
  // one. Reflect used to do `tryParseJson(raw) || {}` and write zero
  // candidates with exit 0 — the same silence that hid coach's defect for ten
  // weeks. The log is still written first so the raw output survives.
  let parsed;
  try {
    assertNotTruncated(response, req.max_tokens, "reflection");
    parsed = parseModelJson(raw, "reflection");
  } catch (e) {
    // A dry run promises no side effects, and that promise holds on the failure
    // path too — coach's failRun already guards on !dryRun, and this did not.
    if (!dryRun) {
      await writeReflectionLog(home, {
        model,
        signalsCount: signals.length,
        proposedCount: 0,
        writtenCount: 0,
        // Literally zero, not a variable: this path fires before the response
        // has been parsed, so no rescore has been attempted or is in scope.
        rescoredCount: 0,
        dryRun,
        rawOutput: raw,
        failure: e.message,
      }).catch((logErr) => {
        console.error(`reflect: could not write the failure log: ${logErr.message}`);
      });
    }
    throw e;
  }

  const rawCandidates = Array.isArray(parsed.candidates) ? parsed.candidates : [];
  const rawRescore = Array.isArray(parsed.rescore) ? parsed.rescore : [];

  // `candidates` holds what was actually accepted this run — and in a
  // non-dry run that means what reached disk. Nothing is pushed before the
  // duplicate checks, so a re-proposed id costs no budget and the reflection
  // log's candidates_written stays honest.
  const candidates = [];
  const rejected = [];
  let skippedCapped = 0;
  let skippedExisting = 0;
  for (const rc of rawCandidates) {
    // The prompt asks for at most `maxCandidates`; this is what makes it true.
    // A model that ignores the instruction must not be able to flood the queue.
    if (candidates.length >= maxCandidates) {
      skippedCapped += 1;
      continue;
    }

    const sanitized = sanitizeCandidate(rc);
    if (!sanitized) {
      rejected.push(typeof rc?.id === "string" ? rc.id : "(no id)");
      continue;
    }

    if (!dryRun) {
      // Skip if a candidate or active lesson with this id already exists —
      // first-write-wins so a flaky model can't clobber a human's promote.
      const cands = await listCandidates(home);
      if (cands.some((c) => c.meta.id === sanitized.meta.id)) { skippedExisting += 1; continue; }
      const lessons = await listLessons(home);
      if (lessons.some((l) => l.meta.id === sanitized.meta.id)) { skippedExisting += 1; continue; }
      await writeCandidate(home, sanitized);
    }

    candidates.push(sanitized);
  }

  // Rescoring runs BEFORE the fail-closed check, and counts toward it, because
  // it is work independent of the candidate list. A response whose candidates
  // were all rejected can still carry valid confidence updates, and aborting
  // ahead of this loop discarded them — the guard against throwing the model's
  // work away, throwing the model's work away.
  const rescored = [];
  if (!dryRun) {
    for (const r of rawRescore) {
      if (!r || typeof r.id !== "string" || !SAFE_ID.test(r.id)) continue;
      if (r.delta !== "confirm" && r.delta !== "contradict") continue;
      const ok = await rescoreLesson(home, r.id, r.delta, config);
      if (ok) rescored.push(r);
    }
  }

  // The same fail-closed rule coach carries, for the same reason — and this is
  // the pass that matters more, because it runs NIGHTLY and unattended. Every
  // proposal must land in exactly one bucket: written, or skipped for a reason
  // we recorded. A run where sanitizeCandidate turned away all of them wrote
  // nothing and, before this, exited 0 — the SOU-40 outcome by another route.
  //
  // The bar is "this run produced nothing at all", not "no candidate landed":
  // a rejected candidate list beside an applied rescore is a productive run,
  // and the rejection is on the record in `candidates_rejected` either way.
  const accountedFor = candidates.length + skippedCapped + skippedExisting + rescored.length;
  if (rawCandidates.length > 0 && accountedFor === 0) {
    const err = new ModelOutputError(
      `reflection: the model proposed ${rawCandidates.length} candidate(s) and none were written ` +
      `or skipped for a recorded reason — all were rejected by validation ` +
      `(${describeRejected(rejected)}). Nothing was written. ` +
      `Check the raw output in the run log.`,
      { kind: "unaccounted" },
    );
    if (!dryRun) {
      await writeReflectionLog(home, {
        model,
        signalsCount: signals.length,
        proposedCount: rawCandidates.length,
        writtenCount: 0,
        rescoredCount: rescored.length,
        rejected,
        dryRun,
        rawOutput: raw,
        failure: err.message,
      }).catch((logErr) => {
        console.error(`reflect: could not write the failure log: ${logErr.message}`);
      });
    }
    throw err;
  }

  const logPath = await writeReflectionLog(home, {
    model,
    signalsCount: signals.length,
    proposedCount: rawCandidates.length,
    // candidates[] holds only what survived sanitization AND the duplicate
    // checks, so this is a real write count. The dry-run path writes nothing.
    // (proposedCount - writtenCount is now readable as "what the cap, the
    // sanitizer and the dedupe together turned away.")
    writtenCount: dryRun ? 0 : candidates.length,
    rescoredCount: rescored.length,
    // A partial loss does not trip the guard above, because something WAS
    // accounted for. Recording it here is what keeps it from being invisible.
    rejected,
    dryRun,
    rawOutput: raw,
  });

  if (!dryRun) {
    const signalBreakdown = signals.reduce((acc, s) => {
      if (s.type === "correction") acc.correction = (acc.correction || 0) + 1;
      else if (s.type === "praise") acc.praise = (acc.praise || 0) + 1;
      else if (s.type === "retry") acc.retry = (acc.retry || 0) + 1;
      return acc;
    }, {});
    try {
      await syncToObsidian({
        home,
        triggeredBy: "reflect",
        pass: {
          kind: "reflect",
          timestamp: new Date().toISOString(),
          signalsConsidered: signals.length,
          signalBreakdown,
          newCandidates: candidates.map((c) => ({ id: c.meta.id, title: c.meta.title })),
          rescoredCount: rescored.length,
        },
      });
    } catch (e) {
      // Sync is best-effort — never fail the reflect pass on a vault hiccup.
      console.error(`obsidian sync failed: ${e.message}`);
    }
  }

  return {
    skipped: false,
    candidates,
    rescored,
    logPath,
    usage: response?.usage,
  };
}

/**
 * Construct an Anthropic SDK client from the resolved API key.
 * Key source order: AGENTMEM_API_KEY env → macOS Keychain → clear error.
 * (See lib/secrets.mjs.) Caller can override for testing.
 */
export async function defaultClient() {
  const { resolveApiKey } = await import("./secrets.mjs");
  const apiKey = await resolveApiKey();
  const { default: Anthropic } = await import("@anthropic-ai/sdk");
  return new Anthropic({ apiKey });
}
