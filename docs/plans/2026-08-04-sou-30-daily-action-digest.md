...
1. The three background jobs that were never scheduled actually run nightly.
...
| `coach` launchd job | **Template only** | `adapters/launchd/com.example.agentmem.coach.plist`; not in `launchctl list` |
...
**Dedupe is not the actual work.** It is worth roughly one item. A semantic pass
may beat lexical, but the design must not assume the backlog collapses.
...
- This is a pre-existing defect, not one this ticket introduces. This plan does
  **not** fix it (that is its own ticket); it routes around it by ordering
  **FIFO on `created`**, which is deterministic, testable, and drains the oldest
  first — matching the ticket's own complaint that the oldest is 4 weeks old.
...
| R3 | `digest orders candidates FIFO by created` | oldest-first | stub preserves directory order |
...
| --- | --- | --- | --- |
| A1 | Two new launchd jobs appear in `launchctl list` | 1 | uninstall one |
...
| A4 | Cap respected; FIFO order | 2 | return unsorted / uncapped |
...
1. **`MEMORY.md` hygiene pass** *(moved up — Probe 4 shows it is days from a
   hard limit)*.
2. **Install the two jobs**: `coach run` from the existing template; author a
   new `ingest --source github` plist.
